...
def _nest_to_proto(nest) -> dataset_info_pb2.SupervisedKeys.Nest:
  """Create a SupervisedKeys.Nest from a limited `tf.nest` style structure.

  Recursive type annotations are not supported yet. The expected type for
  `nest` is:

  Nest = Union[Tuple['Nest', ...], Dict[str, 'Nest'], str]
  ...
  if nest_type is tuple:
    ...
    for key, value in nest.items():
      item = dataset_info_pb2.SupervisedKeys.Dict.Item(
          key=key, value=_nest_to_proto(value))
      proto.dict.items.append(item)
...
def _supervised_keys_to_proto(keys) -> dataset_info_pb2.SupervisedKeys:
...
def _nest_from_proto(proto: dataset_info_pb2.SupervisedKeys.Nest):
  """Create a `tf.nest` style structure from a SupervisedKeys.Nest.
  ...
  if proto.HasField("tuple"):
    ...
    return dict(
        (item.key, _nest_from_proto(item.value)) for item in proto.dict.items)
  elif proto.HasField("feature_key"):
    return proto.feature_key
  else:
    raise ValueError("At least one fieldd of (tuple, list, dict, key)"
                     "must be set on the SupervisedKeys.Nest proto)")
